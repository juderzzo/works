...
def authenticate():
    ...
    use = "giraffe"
    pas = "g"
    # Checks to see if you are inputting values
    if len(request.args) > 0:
        # Stores user's inputs from login page
        username = request.args["user"] + ""
        password = request.args["pass"] + ""
    # If you aren't inputting values and instead just typing /login into the URL it will redirect you back to root
    else:
    ...
    # Else set an error message and redirect back to root
        ...
        error = "Wrong username or password (HINT: username = \"giraffe\", password = \"g\")"
        return redirect(url_for('makeForm'))
